...
- **Add a difficulty stage**: Modify `BibleData::getRevealedText()` logic and update reveal panel in BibirbleWindow UI
- **New Bible book section**: Update AREAS in BOTH `tools/sort.py` AND `src/BibleData.cpp::getBookArea()`
- **UI layout changes**: Edit `BibirbleWindow::SetupUi()` and `SetupKeyboard()` methods
- **Data format changes**: Adjust JSON schema parsing in `BibleData::loadData()` and regenerate `bible_sections.json` via `tools/sort.py`
